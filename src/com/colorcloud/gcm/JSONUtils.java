...
public class JSONUtils {
    public static final String TAG = "GCM_UtilsJSON";
    ...
    public static JSONObject getJsonObject( String jsonstr ) {
    	...
    	try{
    		...
    		GCMLog.e(TAG, "getJsonObject : " + e.toString());
    	}
    	return jsonobj;
    }
    ...
    public static JSONArray getJsonArray(String jsonstr) {
        ...
        try {
        	...
        	GCMLog.e(TAG, "getJSONArray:" + e.toString());
        }
        return curjsons;
    }
    ...
    public static JSONArray truncateJSONArray(JSONArray origarray, int offset){
    	...
    	if( sz > 50){
    		...
    		try{
    			...
    			GCMLog.e(TAG, "truncateJSONArray :" + e.toString());
    		}
    		return newarray;
    	}else{
    ...
    public static int indexOfJSONObject(JSONArray jsonarray, JSONObject jsonobj, String key) {
        ...
        if (key == null) {
            ...
            try {
                ...
                objstr = null;
                GCMLog.e(TAG, "findJSONObject:  get key Exception: " + e.toString());
            }
        }
        ...
        if (objstr != null) {
        	...
        	GCMLog.d(TAG, "findJSONObject:  empty key string! no found. ");
            return -1;
        }
        ...
        for (int i=0; i<size; i++) {
            try {
                ...
                if (objstr.equals(entrystr)) {
                	GCMLog.d(TAG, "findJSONObject: match :" + objstr);
                    return i;   // return immediately
                }
            } catch (JSONException e) {
            	GCMLog.e(TAG, "findJSONObject: getJSONObject Exception: " + e.toString());
                continue;
            }
        }
        return -1;
    ...
    public static JSONArray mergeJsonArrays(JSONArray existingjsons, JSONArray newjsons, boolean updatess) {
        ...
        for (int i=0; i<newjsons.length(); i++) {
            try {
                ...
                int idx = indexOfJSONObject(existingjsons, newobj, SENDER);
                if(idx < 0) { // not found, insert newjson obj into json array
                    ...
                    if(newobj.has(SENDER)) {
                        String newss = newobj.getString(SENDER);
                        JSONObject oldobj = existingjsons.getJSONObject(idx);
                        oldobj.put(SENDER, newss);
                        GCMLog.d(TAG, "mergeJsonArrays: update ss: " + newss + " : " + oldobj.toString());
                    }
                }
            } catch (JSONException e) {
                GCMLog.e(TAG, "mergeJSONArrays: getJSONObject Exception: " + e.toString());
                continue;
            }
        }
        return existingjsons;
    ...
    public static String mergeJsonArrayStrings(String curstr, String newstr) {
        ...
        GCMLog.d(TAG, "mergeJSONArrays:" + curstr + " =+= " + newstr);
        ...
        try {
        	...
        	GCMLog.e(TAG, "mergeJSONArrays:" + e.toString());
    ...
    public static boolean fuzzyMatchJsonArrays(String dbJsonStr, String curJsonStr, String key) {
    	GCMLog.d(TAG, "fuzzyMatchJSONArrays : dbsdbjsonstret : " + dbJsonStr + " : curjsonstr :" +curJsonStr);
    ...
    public static Set<String> getValueSetFromJsonArray(JSONArray jsonarray, String key) {
        ...
        for (int i=0; i<jsonarray.length(); i++) {
            try {
            	...
            	GCMLog.e(TAG, "getValueSetFromJSONArray: Exception: " + e.toString());
                continue;  // skip this entry if can not construct object.
            }
        }
        return valset;
        ...
        if (jsonString != null && jsonString.length() > 0) {
            for (String s : set) {          
                ...
                if (jsonString.indexOf(wrapstr) >= 0) {
                    GCMLog.d(TAG, "intersectSetJsonArray: Match: " + wrapstr);
                    matchcnt++;
                }
            }
        }
